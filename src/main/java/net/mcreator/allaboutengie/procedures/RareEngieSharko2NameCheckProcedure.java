			...
			if (entity.getPersistentData().getDouble("sharkoZoomiesTimer") >= 150) {
				if (entity instanceof EngieSharkoRare2TamedZoomiesEntity) {
					...
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = AllaboutengieModEntities.ENGIE_SHARKO_RARE_2_TAMED_ZOOMIES_TIRED.get().spawn(_level, BlockPos.containing(x, Math.ceil(y), z), MobSpawnType.MOB_SUMMONED);
			...
			if (entity.getPersistentData().getDouble("sharkoZoomiesTiredTimer") >= 150) {
				if (entity instanceof EngieSharkoRare2TamedZoomiesTiredEntity) {
					if (!entity.level().isClientSide())
						entity.discard();
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = AllaboutengieModEntities.ENGIE_SHARKO_RARE_2_TAMED.get().spawn(_level, BlockPos.containing(x, Math.ceil(y), z), MobSpawnType.MOB_SUMMONED);
						if (entityToSpawn != null) {
							entityToSpawn.setYRot(entity.getYRot());
							entityToSpawn.setYBodyRot(entity.getYRot());
							entityToSpawn.setYHeadRot(entity.getYRot());
